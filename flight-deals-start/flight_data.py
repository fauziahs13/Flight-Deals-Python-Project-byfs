...
def find_cheapest_flight(flights_data):
    ...
    multiple entries.
    Args:
        flights_data (dict): The JSON data containing flight information returned by the API.
    Returns:
    ...
    # ^ If you go from London to Bali the stop (let's say) Singapore.
    # Then the destination we get = DPS which the final destination (for I Gusti Airport) not SIN (for Changi airport)
    ...
    """ 
    Now we can make a flight data object by passing these variables as parameters so we can initialize with the 
    first flight in the json
    """
